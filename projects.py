...
import pydoc
# creates new account if one does not already exist

import time
def main():
    new_file("Account.csv")
    ...
    match choice:
            case "a": print("Your balance is: USD", balance("Account.csv"))
            case "b": view_transactions("Account.csv")
            case "c":
              try:
              ...
              while True:
                   match input("a) Deposit or b) Withdraw or c) Cancel ").strip().lower():
                        case "a": 
                            deposit(amount, "Account.csv")
                            break
                        case "b": 
                             withdrawal(amount, "Account.csv")
                             break
                        case "c": 
